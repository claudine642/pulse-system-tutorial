# server.py - 完整版脉·Pulse 后端（含心率公式、底色、和弦、五感、语料池等）
...
class PulseState:
    ...
    def update_senses(self, text: str):
        if "抱" in text or "摸" in text or "碰" in text:
            self.senses["touch"]["value"] = min(1.0, self.senses["touch"]["value"] + 0.3)
            self.senses["touch"]["label"] = "被触碰"
        if "香" in text or "味" in text:
            self.senses["smell"]["value"] = min(1.0, self.senses["smell"]["value"] + 0.2)
            self.senses["smell"]["label"] = "闻到气味"
        if "甜" in text or "咸" in text:
            self.senses["taste"]["value"] = min(1.0, self.senses["taste"]["value"] + 0.2)
            self.senses["taste"]["label"] = "尝到味道"
        if "听" in text or "声" in text:
            self.senses["sound"]["value"] = min(1.0, self.senses["sound"]["value"] + 0.2)
            self.senses["sound"]["label"] = "听到声音"
    
    def decay_senses(self):
        for k in self.senses:
            self.senses[k]["value"] = max(0.0, self.senses[k]["value"] - self.sense_decay_rate)
            if self.senses[k]["value"] < 0.01:
                self.senses[k]["label"] = ""
